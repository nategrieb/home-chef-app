...
export default function RecipeDetail() {
  ...
  const saveChanges = async () => {
    ...
    if (!error) {
      ...
      const ings = ingredients.map(ing => ({ ...ing, recipe_id: params.id, calories_per_unit: 0 }));
  ...
  return (
    <main className="min-h-screen bg-white pb-40 px-6 pt-10">
      <header className="flex justify-between items-start mb-8 gap-4">
        ...
        <button onClick={() => isEditing ? saveChanges() : setIsEditing(true)} className="bg-[#004225] text-white px-6 py-3 rounded-2xl font-black uppercase text-xs shadow-lg active:scale-95 transition-all shrink-0">
          {isEditing ? "Save" : "Edit"}
        </button>
      </header>
        ...
        {isEditing ? (
          <div className="space-y-4">
            <textarea 
              ...
              className="w-full bg-white p-4 rounded-2xl text-black font-bold outline-none border-2 border-slate-300 min-h-[120px]" 
              style={{ color: '#000000', backgroundColor: '#FFFFFF' }}
            ...
            <div key={i} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
              {isEditing ? (
                <div className="flex gap-2 w-full">
                  <input value={ing.item_name} onChange={e => {
                    const n = [...ingredients]; n[i].item_name = e.target.value; setIngredients(n);
                  }} className="flex-1 bg-white border border-slate-300 p-2 rounded-lg font-black text-black outline-none" style={{ color: '#000000', backgroundColor: '#FFFFFF' }} />
                  <input value={ing.amount} onChange={e => {
                    const n = [...ingredients]; n[i].amount = e.target.value; setIngredients(n);
                  }} className="w-16 bg-white border border-slate-300 p-2 rounded-lg text-center font-black text-black outline-none" style={{ color: '#000000', backgroundColor: '#FFFFFF' }} />
